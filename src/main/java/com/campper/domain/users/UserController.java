...
public class UserController {
    ...
    public void putUser(@RequestBody @Valid PutUserPwdDto putUserPwdDto, @AuthenticationPrincipal String authKey) {
        userService.updatePwd(putUserPwdDto, authKey);
    }

    @PutMapping("/profile")
    @Operation(summary = "프로필 수정 요청", description = "프로필 수정 요청 API 입니다.")
    public GetUserProfileDto putProfile(@RequestBody @Valid PutUserProfileDto putUserProfileDto, @AuthenticationPrincipal String authKey) {
        return userService.updateProfile(putUserProfileDto, authKey);
    }

    @GetMapping("/profile")
    @Operation(summary = "프로필 요청", description = "프로필 요청 API 입니다.")
    public GetUserProfileDto getProfile(@AuthenticationPrincipal String authKey) {
        return userService.getUserProfile(authKey);
    }

    @DeleteMapping("/")
    @Operation(summary = "회원 탈퇴 요청", description = "회원 탈퇴 API 입니다.")
    public void delUser(@RequestBody @Valid DelUserDto delUserDto, @AuthenticationPrincipal String authKey) {
        userService.withdraw(delUserDto, authKey);
    }
}
